...
### 1.1 🟡 Integrate `@mlc-ai/web-llm` for real on-device LLM
...
### 1.2 🟢 Wire the audio pipeline into HoldToTalkOrb (voice notes, real)
...
### 1.3 🟡 On-device transcription (Whisper.cpp WASM / transformers.js)
...
### 1.4 🟢 Replace the fake face-expression worker with a real model
...
### 1.5 🟢 Make CalmingAudio actually binaural
...
### 1.6 🟢 Real TTS (or keep honest badge)
...
### 1.7 🟢 Wire waveform playback + real barge-in
...
### 2.4 🟢 Real server-side export path (wire the dead `uploadExportBundle`)
...
### 2.5 🟢 Session detail: show the real chat transcript
...
### 2.6 🟢 Kanban status changes persist
...
### 2.7 🟢 Memory edits / verification persist to backend
...
### 3.4 🟢 Edge-delete endpoint (see 2.7)
Backend has node delete only. Add `DELETE /memory/edge/:id` to match `unlink`.
...
> Fast wins first: **1.4** (face), **1.5** (binaural), **1.6** (TTS), **2.1** (semantic search UI), **2.2** (add-memory UI), **2.6** (kanban persist) — each is self-contained, needs no new backend, and removes a "fake" from the audit.
