...
## Error Standardization — ADR-008 (2026-08-18)

Standardisasi penanganan error full-stack: satu taxonomy + envelope + choke point, dipropagasi ke **CloudWatch + LGTM** (Grafana Loki/Tempo/Mimir). **Semua VERIFIED** (lambda 157/157 ✓, frontend 87/87 ✓, typecheck+build bersih). Kontrak lengkap: `docs/15-8-26/ADR-008-error-standardization.md`.

- [x] **Taxonomy + envelope** (`lambda/lib/errors.ts` baru) — katalog `ERROR_CODES` (kode stabil per domain → statusCode / category `client|validation|dependency|internal` / retriable / level `warn|error`); `AppError` + `fail()`; client hanya menerima `{error:{code,message,retriable}}` (details/cause/statusCode **tidak pernah** bocor); error tak dikenal → `internal.unhandled` (500 retriable).
- [x] **Choke point tunggal** `reportError(err,{span?,route?})` — tepat **satu emisi per kegagalan**: `logger.error` (→CloudWatch) + `emitLog` (→Loki via OTLP) + `recordError` (→Mimir counter `app.error.count`, label tertutup `error.code`/`error.category`/status class) + `span.recordException`+`setStatus(ERROR)` (→Tempo). Semua 12 handler + catch terpusat `handler.ts` dikonversi; frame SSE chatTurn (`code:"chat_turn_failed"`) dipertahankan persis (test-critical).
- [x] **Perbaikan leak** — `export.ts` tidak lagi membocorkan `err.message` ke client (envelope `export.failed`); semantik HTTP dipetakan katalog (401 auth.*, 503 dependency.*, 502 telemetry, 413 payload, 404 resource/media.not_found).
- [x] **Frontend klasifikasi** (`src/shared/lib/errors.ts` baru) — `ApiError` + `classifyFetchError`: `TypeError "Failed to fetch"` → `network.unreachable` (pesan CORS/CSP diagnosable), `AbortError` → `network.request_aborted`, `BackendErrorFrame` → `chat_turn_failed`, `RateLimitError` → internal retriable; `apiClient` parse envelope backend → toast error (mis. CameraPip "Index failed") kini menampilkan sebab nyata. Ter-wire di `apiClient.ts` (api() + uploadMediaToS3) + `CameraPip.tsx`.
...
- [x] **Ship — E2E terbukti** (2026-08-18): seperti di atas. Trace chat turn `a3f02149...`/`6722800013e29c...` (04:13–04:14Z) masuk **kedua sink dengan trace_id sama**; **Phoenix** punya `gen_ai.request.input` penuh (prompt system+user) + model `openrouter/free` + `span_kind=LLM`; **Tempo** punya span yang sama HANYA `gen_ai.operation.name=chat` + `openinference.span.kind=LLM` → **payload prompt TER-STRIP** di Tempo (governance jalan). Trace penuh: `chat/turn` → `llm.openrouter` ×2 + `llm.embedding` ×2 + `agent.memory.retrieve` ×2 + `db.query` ×34.
- [x] **Fix bendera** — Tempo sempat kosong total setelah dual-sink (deploy `2840cb6`): `StrippingExporter` menyalin span via object spread `{...span}` yang membuang `SpanImpl.prototype` → `otlp-transformer` panggil `span.spanContext()` → `TypeError` sinkron → `BatchSpanProcessor` membuang batch diam-diam (sink Phoenix aman karena pakai objek original). Diperbaiki di `afe932b`: klon dengan `Object.assign(Object.create(Object.getPrototypeOf(span)), span, { attributes })` + 3 regression test (prototype hidup, payload ter-strip, sumber tidak termutasi) → **181/181** ✓, CI green (#32098015898), trace health/db/chat mengalir kembali ke Tempo. 
