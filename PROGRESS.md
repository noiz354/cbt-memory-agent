...
- [ ] **Masih terbuka** — `getMemoryContext` (chat retrieval) tetap heuristic weight/confidence (bukan vector); vector search hanya di endpoint `/memory/semantic`. Menambahkan hybrid retrieval ke chat turn = follow-up terpisah (dokumentasi di riset). EXPLAIN dengan 2 baris memilih full-scan+sort (cost-based optimal untuk data kecil); C-SPANN aktif otomatis saat data tumbuh.
