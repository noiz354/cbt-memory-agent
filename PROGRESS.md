...
- [x] **Deploy live + verifikasi** — migration CRDB live (`attachments` table + `kind CHECK ('core','transcript','attachment')` terverifikasi di cluster), `terraform apply` (Lambda v19, IAM S3 delete), **live E2E** presign→PUT 200→create→list→delete (S3 + cascade), fix presigned PUT signature (commit `68a6dcb`), reflection gate live (`CLUSTER_HEALTH_CHECK` audit row). Recall attachment via hybrid RRF tinggal diverifikasi manual lewat UI/frontend live.
