...
- [x] **Docs** — `docs/15-8-26/PLAN-MCP-REFLECTION-STEP.md` + `docs/15-8-26/PLAN-CLUSTER-HEALTH-SKILLS.md` (Define→Plan→Build→Verify→Review→Ship, hasil verifikasi tertera), `docs/MCP-STATUS.md` §3 (cluster health gate) + §4 (skills injection). **Sisa**: `terraform apply` + verifikasi live behavior di Lambda (belum di-deploy).
...
- [x] **Review & docs** — code-review 5-axis → fix delete-by-nodeId (sebelumnya `attachments.id` tidak pernah cocok dengan id yang dikembalikan create → selalu 404), validasi ext presign, harden analyze worker. **Sisa**: `terraform apply` + migration CRDB live + verifikasi recall di cluster nyata (belum di-deploy).
